...
//-------------------------------------------------------------------
 	case 'GET':
 				...
 				if(!empty($_GET)){
					foreach ($_GET as $key=>$val) {
					  switch ($key) {
					  	case 'post_num':
							...
							$q = mysql_query("SELECT * FROM posts WHERE post_num = '$post_no' AND post_type = 'quote' LIMIT 1");
					  		break;

					  	case 'latest':
					  		$amount = $_GET['1'];
							$q = mysql_query("SELECT * FROM posts WHERE post_type = 'quote' ORDER BY post_num DESC LIMIT 1");
					  		break;
					...
					$q = mysql_query("SELECT * FROM posts WHERE post_type = 'quote' ORDER BY post_num DESC");
				}
